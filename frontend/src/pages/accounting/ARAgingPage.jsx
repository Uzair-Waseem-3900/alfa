...
const columns = [
    ...
    },
    {
        ...
        render: (value) => value > 0
            ? <span className="font-semibold text-error-600">{value}</span>
            : <span className="text-neutral-400">Not due</span>,
    },
    {
        key: 'bucket',
        label: 'Aging Bucket',
...
const ARAgingPage = () => {
    ...
    } = usePaginatedList(accountingApi.arAging.get, {});
        ...
        <div className="space-y-6">
            <div className="p-4 bg-amber-50 border-l-4 border-amber-500 rounded-r-xl flex gap-3">
                ...
                <div>
                    ...
                    <p className="text-sm text-amber-700 mt-0.5">
                        Bucketed by each invoice's due date against today. Always independently verify
                        before making collection or credit decisions.
                    </p>
                </div>
            </div>
            ...
            {loading ? (
                ...
                <>
                            ...
                            {Object.entries(BUCKET_LABELS).map(([key, label]) => (
                                ...
                                >
                                    ...
                                    <p className="text-xs text-neutral-400 mt-0.5">
                                        {summary.buckets?.[key]?.count ?? 0} invoice(s)
                                    </p>
                                </Card>
                            ))}
                        </div>
                    ...
                    {results.length === 0 ? (
                        <EmptyState
                            title={activeBucket ? 'No Invoices In This Bucket' : 'No Outstanding Invoices'}
                            description={activeBucket ? 'Try a different bucket or clear the filter.' : 'Every confirmed invoice is fully paid.'}
                        />
                    ) : (
                        <>

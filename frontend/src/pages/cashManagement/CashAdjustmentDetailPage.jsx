...
const CashAdjustmentDetailPage = () => {
    ...
    const fetchAdjustment = useCallback(async () => {
        ...
        try {
            ...
            if (error?.response?.status === 404) {
                ...
                setLoadError(extractErrorMessage(error, 'Failed to load cash adjustment'));
            }
            setAdjustment(null);
        } finally {
            setLoading(false);
    ...
    if (notFound) {
        return (
            <div className="text-center py-16">
                ...
                <BackLink to="/cash-management/adjustments" className="mt-4 inline-flex">Back to Cash Adjustments</BackLink>
            </div>
        );
    }

    if (loadError || !adjustment) {
        return (
            <div className="space-y-4">
                <BackLink to="/cash-management/adjustments">Back to Cash Adjustments</BackLink>
                <InlineAlert
                    variant="error"
                    title="Couldn't load this cash adjustment"
    ...
    const ImpactIcon = isLost ? ArrowDownCircle : ArrowUpCircle;
    ...
    return (
        <div className="space-y-6">
            ...
            <Card className="p-6">
                <h3 className="font-semibold text-neutral-900 mb-4">Adjustment Information</h3>
                <div className="grid grid-cols-2 md:grid-cols-3 gap-5">
                    <InfoField
                        label="Amount"
                        value={<span className={isLost ? 'text-error-600' : 'text-success-600'}>Rs. {fmt(adjustment.amount)}</span>}
                    />
                    <InfoField icon={Calendar} label="Date" value={new Date(adjustment.adjustment_date).toLocaleDateString()} />
                    <InfoField icon={User} label="Recorded By" value={adjustment.created_by || 'N/A'} />
                    <InfoField icon={Clock} label="Created At" value={new Date(adjustment.created_at).toLocaleString()} />
                    {adjustment.reason && (
                        <div className="col-span-full">
                            <InfoField icon={FileText} label="Reason" value={adjustment.reason} />
                        </div>
                    )}
                </div>
            </Card>

            {adjustment.allocations?.length > 0 && (
                <Card className="p-6">
            ...
            <Card className="p-6">
                ...
                <div className={`flex items-start gap-3 p-4 rounded-xl border-l-4 ${isLost ? 'bg-warning-50 border-warning-500' : 'bg-success-50 border-success-500'}`}>
                    <ImpactIcon className={`w-5 h-5 flex-shrink-0 mt-0.5 ${isLost ? 'text-warning-600' : 'text-success-500'}`} />
                    <div>
                        <p className={isLost ? 'text-warning-700' : 'text-success-700'}>
                            This {isLost ? 'reduced' : 'increased'} cash in hand by <strong>Rs. {fmt(adjustment.amount)}</strong>
                        </p>
                        <p className={`text-sm mt-1 ${isLost ? 'text-warning-600' : 'text-success-600'}`}>
